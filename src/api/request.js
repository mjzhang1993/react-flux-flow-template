...
// 删除底部 '/'
function deleteSlash(host: string) {
   return host.replace(/\/$/, '');
}
...
// 解析参数
function separateParams(url: string) {
...
// 一些常用的请求方法
export const get = (url: string, data?: Object) => request({ url, data });
export const post = (url: string, data?: Object) => request({ method: 'POST', url, data });
export const put = (url: string, data?: Object) => request({ method: 'PUT', url, data });
export const del = (url: string, data?: Object) => request({ method: 'DELETE', url, data });
